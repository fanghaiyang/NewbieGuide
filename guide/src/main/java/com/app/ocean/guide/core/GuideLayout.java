...
public class GuideLayout extends FrameLayout {
    ...
    private OnGuideLayoutShipDismissListener listenerSkip;
    private float downX;
    private float downY;
    ...
    private void addCustomToLayout(GuidePage guidePage) {
        ...
        if (layoutResId != 0) {
            final View view = LayoutInflater.from(getContext()).inflate(layoutResId, this, false);
            ...
            int viewId = guidePage.getClickToSkipId();
            View click = view.findViewById(viewId);
            if (click != null) {
                click.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        if (listenerSkip != null) {
                            listenerSkip.onGuideLayoutShipDismiss();
                        }
                        remove();
//                        Toast.makeText(getContext(),"onGuideLayoutDismiss2",Toast.LENGTH_LONG).show();
                    }
                });
            } else {
                Log.w(NewbieGuide.TAG, "can't find the view by id : " + viewId + " which used to remove guide page");
    ...
    public interface OnGuideLayoutShipDismissListener {
        void onGuideLayoutShipDismiss();
    }

    public void setOnGuideLayoutSkipDismissListener(OnGuideLayoutShipDismissListener listener) {
        this.listenerSkip = listener;
    }
}
